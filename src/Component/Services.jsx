      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {services.map((service) => (
          <div className="relative group overflow-hidden rounded-[16px]">
            ...
            <p className="text-[14px] absolute inset-0 bg-black bg-opacity-70 text-base text-white p-4 flex flex-col justify-center opacity-0 translate-y-full group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500 ease-in-out leading-relaxed">
              {service.desc}
            </p>

            {/* Title fixed at the bottom initially and moves to the top on hover */}
            <h2 className="p-4 text-center absolute bottom-0 left-0 right-0 text-base text-lg sm:text-xl font-bold py-2 transition-transform duration-500 ease-in-out transform group-hover:top-0 group-hover:bottom-auto group-hover:translate-y-0">
              {service.title}
            </h2>
          </div>
        ))}
